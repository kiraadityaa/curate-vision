...
class PipelineConfig(BaseModel):
    ...
    min_file_size: int = Field(
        default=0, ge=0, description="Min bytes (0 = disabled)."
    )
    ...
    # --- Pipeline ---
    checkpoint_every: int = Field(default=500, ge=0, description="Save checkpoint every N images.")
    batch_size: int = Field(default=32, ge=1)
    workers: int = Field(default=0, ge=0, description="Dataloader workers (0 = main thread).")
